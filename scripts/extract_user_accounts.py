...
def extract_user_accounts():
    ...
    auth_params = {"user": "analytics", "password": "analytics123"}
    ...
    for tx in transactions:
        ...
        if sender_id and sender_id not in users:
            users[sender_id] = {
                ...
                "email": (
                    f"{sender_id.split('_')[1][:8]}@cryptobridge.com"
                    if "_" in sender_id
                    else f"{sender_id[:8]}@cryptobridge.com"
                ),
                "registration_date": datetime.now().date().strftime("%Y-%m-%d"),
                "risk_profile": (
                    "high"
                    if tx.get("is_suspicious", 0) == 1
                    else ("medium" if tx.get("risk_score", 0) > 50 else "low")
                ),
        ...
        if receiver_id and receiver_id not in users:
            users[receiver_id] = {
                ...
                "email": (
                    f"{receiver_id.split('_')[1][:8]}@cryptobridge.com"
                    if "_" in receiver_id
                    else f"{receiver_id[:8]}@cryptobridge.com"
                ),
                "registration_date": datetime.now().date().strftime("%Y-%m-%d"),
                "risk_profile": (
                    "high"
    ...
    for tx in transactions:
        ...
        receiver_id = tx.get("receiver_user_id")
        if receiver_id:
            user_volumes[receiver_id] += (
                amount_usd * 0.5
            )  # Receiving is less volume impact
    ...
    for user_id, user_data in users.items():
        ...
        user_data["preferred_cryptos"] = list(user_cryptos.get(user_id, {"BTC"}))[
            :3
        ]  # Top 3
    ...
    for i in range(0, len(user_list), batch_size):
        ...
        for user in batch:
            ...
            values.append(
                f"('{user['user_id']}', '{user['account_type']}', '{user['email']}', "
        ...
        try:
            response = requests.post(
                base_url, params=auth_params, data=insert_query, timeout=30
            )

            if response.status_code == 200:
                total_inserted += len(batch)
                print(
                    f"   ⏳ Inserted {total_inserted:,}/{len(user_list):,} user accounts..."
                )
            else:
                print(
                    f"⚠️ Batch insert failed: {response.status_code} - {response.text[:200]}"
                )
        ...
        if response.status_code == 200:
            ...
            if response.status_code == 200:
                ...
                for row in result.get("data", []):
                    print(
                        f"   • {row['account_type']} in {row['country']}: {row['count']} users"
                    )
