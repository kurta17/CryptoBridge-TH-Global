...
class CryptoBridgeDataGenerator:
    ...
    def _initialize_accounts(self):
        ...
        for i in range(20):
            account = {
                ...
                'country': random.choice(['US', 'UK', 'CN', 'KR', 'JP', 'SG', 'CA', 'DE']),
                'registration_date': fake_global.date_between(start_date='-3y', end_date='-6m'),
                'kyc_level': 3,
                'preferred_cryptos': random.sample(['BTC', 'ETH', 'USDT', 'USDC', 'BNB'], 3),
                'monthly_volume_usd': random.uniform(500000, 5000000),  # $500K - $5M
                'risk_profile': 'low' if i < 15 else 'medium'  # Most whales are low risk
            }
        ...
        for i in range(200):
            account = {
                ...
                'email': fake_global.email(),
                'name': fake_global.name(),
                'country': random.choice(['US', 'UK', 'CN', 'KR', 'JP', 'SG', 'MY', 'VN', 'PH', 'ID']),
                'registration_date': fake_global.date_between(start_date='-2y', end_date='-1w'),
                'kyc_level': random.choices([1, 2, 3], weights=[0.2, 0.5, 0.3])[0],
                'preferred_cryptos': random.sample(['BTC', 'ETH', 'USDT', 'USDC', 'BNB', 'ADA', 'DOT'], 2),
                'monthly_volume_usd': random.uniform(1000, 100000),  # $1K - $100K
                'risk_profile': random.choices(['low', 'medium'], weights=[0.8, 0.2])[0]
            }
        ...
        for i in range(10):
            account = {
                ...
                'email': fake_global.email(),
                'name': fake_global.name(),
                'country': random.choice(['US', 'RU', 'CN', 'VE', 'IR']),  # Higher risk countries
                'registration_date': fake_global.date_between(start_date='-6m', end_date='-1d'),
                'kyc_level': 1,  # Low KYC level
                'preferred_cryptos': ['USDT', 'USDC', 'BTC'],  # Privacy coins preference
                'monthly_volume_usd': random.uniform(100000, 1000000),  # High volume, low KYC
                'risk_profile': 'high',
        ...
        for i in range(150):
            receiver = {
                ...
                'registration_date': fake_thailand.date_between(start_date='-2y', end_date='-1w'),
                'preferred_cryptos': random.sample(['BTC', 'ETH', 'USDT', 'USDC', 'BNB'], 2),
                'account_type': random.choices(['regular', 'business'], weights=[0.7, 0.3])[0]
            }
            self.thai_receivers.append(receiver)
        
        print(f"✅ Generated {len(self.whale_accounts)} whale, {len(self.regular_accounts)} regular, "
    ...
    async def fetch_real_crypto_data(self) -> Dict[str, Any]:
        ...
        async with aiohttp.ClientSession() as session:
            try:
                ...
                async with session.get(f"{self.binance_base_url}/ticker/price") as response:
                ...
                try:
                    async with session.get(f"{self.binance_th_url}/ticker/price") as response:
                        if response.status == 200:
                            th_price_data = await response.json()
                            for item in th_price_data:
                                symbol = item['symbol']
                                if symbol in thb_pairs:
                                    crypto_data['thb_pairs'][symbol] = float(item['price'])
                except:
                ...
                async with session.get(f"{self.binance_base_url}/ticker/24hr") as response:
                    if response.status == 200:
                ...
                print(f"✅ Fetched {len(crypto_data['prices'])} price points, "
    ...
    def generate_cross_border_transactions(self, 
                                         ...
                                         days_back: int = 30,
                                         fraud_percentage: float = 0.05) -> List[Dict[str, Any]]:
        ...
        for i in range(normal_count):
            ...
            transaction = self._create_transaction(sender, receiver, crypto, base_amount_usd, timestamp)
        ...
        fraud_transactions = self._generate_fraud_transactions(fraud_count, start_date, end_date)
        transactions.extend(fraud_transactions)
        
        # Sort by timestamp
        transactions.sort(key=lambda x: x['timestamp'])
        
        print(f"✅ Generated {len(transactions)} total transactions ({fraud_count} fraudulent)")
        return transactions
    
    def _generate_business_timestamp(self, start_date: datetime, end_date: datetime) -> datetime:
    ...
    def _create_transaction(self, sender: Dict, receiver: Dict, crypto: str, 
                          amount_usd: float, timestamp: datetime, 
                          is_suspicious: bool = False) -> Dict[str, Any]:
        """Create a standardized transaction record"""
        
        # Get crypto price (use cached prices or fallback)
        crypto_price_usd = self.crypto_prices.get('prices', {}).get(f"{crypto}USDT", {
            'BTC': 45000, 'ETH': 2800, 'USDT': 1, 'USDC': 1, 'BNB': 320
        }.get(crypto, 100))
        ...
        risk_score = self._calculate_risk_score(sender, receiver, amount_usd, is_suspicious)
    ...
    def _generate_fraud_transactions(self, count: int, start_date: datetime, end_date: datetime) -> List[Dict]:
        ...
        for i in range(count):
            ...
            if scenario['name'] == "High Frequency Large Transfers":
                ...
                for j in range(num_transfers):
                    ...
                    receiver = random.choice(self.thai_receivers)
                    amount_usd = random.uniform(8000, 12000)  # Just under typical limits
    ...
    def _calculate_risk_score(self, sender: Dict, receiver: Dict, amount_usd: float, is_suspicious: bool) -> int:
        ...
        if self.crypto_prices.get('trading_volumes'):
            for symbol, stats in self.crypto_prices['trading_volumes'].items():
                ...
                trading_data['volume_comparison'][symbol] = {
                    ...
                    'th_daily_trades': int(stats['count'] * 0.1),  # TH is ~10% of global
    ...
    async def save_datasets(self, transactions: List[Dict], trading_data: Dict):
        ...
        print(f"   📄 crypto_prices.json: {len(self.crypto_prices.get('prices', {}))} prices")
    
    async def generate_business_summaries(self, transactions: List[Dict], trading_data: Dict):
        ...
        summaries = {
            'cross_border_volume': {
                ...
                'top_crypto_currencies': df['crypto_currency'].value_counts().head(5).to_dict(),
                'daily_volumes': df.groupby(df['timestamp'].str[:10])['fiat_amount'].sum().to_dict()
            },
            'top_senders': df.groupby('sender_user_id').agg({
                'fiat_amount': 'sum',
                'transaction_id': 'count'
            }).sort_values('fiat_amount', ascending=False).head(10).to_dict(),
            'top_receivers': df.groupby('receiver_user_id').agg({
                'fiat_amount': 'sum',
                'transaction_id': 'count'
            }).sort_values('fiat_amount', ascending=False).head(10).to_dict(),
            'fraud_analysis': {
                'total_suspicious': len(df[df['is_suspicious'] == 1]),
                'suspicious_volume_usd': df[df['is_suspicious'] == 1]['fiat_amount'].sum(),
    ...
    def print_generation_report(self, data: Dict):
        ...
        print(f"   • Cross-border Volume: ${sum(t['fiat_amount'] for t in transactions):,.2f}")
        print(f"   • Suspicious Transactions: {sum(1 for t in transactions if t['is_suspicious']):,}")
