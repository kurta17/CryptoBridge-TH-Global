...
class ClickHouseLoader:
    def __init__(self):
        ...
        self.auth_params = {"user": "analytics", "password": "analytics123"}
    ...
    def setup_enhanced_schema(self):
        ...
        for table_name, table_sql in tables:
            try:
                response = requests.post(
                    self.base_url, params=self.auth_params, data=table_sql, timeout=15
                )
                if response.status_code == 200:
                    print(f"✅ Table '{table_name}' created/verified")
                else:
                    print(
                        f"⚠️ Table '{table_name}' response: {response.status_code} - {response.text}"
                    )
            except Exception as e:
            ...
            if "volume_comparison" in trading_data:
                ...
                if all_values:
                    ...
                    if response.status_code == 200:
                        ...
                        print(
                            f"⚠️ Trading volumes loading failed: {response.status_code}"
                        )
    ...
    def verify_data_loading(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                data = result["data"][0]
                print(
                    f"   ✅ Cross-border Analysis: {data['total_transactions']:,} transactions, "
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                data = result["data"][0]
                print(
                    f"   ✅ Fraud Detection: {data['suspicious_count']:,} suspicious transactions, "
